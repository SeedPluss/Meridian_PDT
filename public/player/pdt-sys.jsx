            ...
            {systems.map(sys => (
              <div key={sys.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px' }}>
                  <span style={{
                    ...vt(18, (sys.status === 'online' || sys.online) ? C.main : sys.repairing ? C.amber : (!sys.panelUnlocked && sys.locked) ? C.dim : C.dim),
                    flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'
                  }}>
                    {sys.label}
                  </span>
                  <span style={{
                    ...vt(16, (sys.status === 'online' || sys.online) ? C.bright : sys.repairing ? C.amber : C.dim),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          {isAndroid && diag && (
            <>
              <AsciiRule />
              <div style={{
                border: `1px solid ${C.cyan}`, padding: '10px 12px',
      ...
      {success ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', margin: '12px 0' }}>
          ...
          {REVELATIONS[system?.id] && (
            <div style={{
              marginTop: '12px', border: `1px solid ${C.dim}`, padding: '10px',
              background: C.ghost
            }}>
